...
async function migrateExistingTalks() {
	try {
		...
		result1.forEach((talk) =>
			console.log(`  - ${talk.title} (ID: ${talk.id})`),
		);
		...
		allTalks.forEach((talk) => {
			console.log(
				`ID: ${talk.id}, SessionID: ${talk.sessionId || "NULL"}, Date: ${talk.presentationDate}, Title: ${talk.title}`,
			);
		});
	} catch (error) {
