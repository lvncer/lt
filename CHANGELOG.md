...
- 追加
  ...
  - 特別枠セッション機能の追加（セッション番号を持たない特別なセッションに対応）
  - セッション作成時に種別（通常枠/特別枠）を選択できる UI を追加
  - 特別枠セッションの自動タイトル生成機能（「日付 特別枠」形式）

- 変更

  - データベーススキーマに `is_special` フィールドを追加し、`session_number` を NULL 許容に変更
  - セッション番号の重複チェックを通常枠のみに適用するよう変更
  - セッションフォームに条件付きバリデーションを追加（特別枠の場合はセッション番号不要）
  - API レスポンスに `isSpecial` フィールドを追加（`/api/available-sessions`, `/api/schedule-dates`）

- 修正
  - Register ページ、Dashboard、スケジュールページでの特別枠セッション表示を修正（「第 null 回」→「特別枠」）
  - セッション選択ドロップダウンで特別枠を「特別枠 - 日付 (会場)」形式で表示
  - セッション削除時のトーストメッセージを特別枠に対応
  - エラーメッセージの表示スタイルを赤色に統一（フィールド内エラー、必須マーク）
